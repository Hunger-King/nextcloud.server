OC.L10N.register(
    ...
    "Necessary language model provider is not available" : "沒有須要的語言模型提供者",
    ...
    "Enter your subscription key in the support app in order to increase the user limit. This does also grant you all additional benefits that Nextcloud Enterprise offers and is highly recommended for the operation in companies." : "請在支援應用程式中輸入您的訂閱方案金鑰，以提高使用者限制。這也能賦予您 Nextcloud 企業版提供的所有其他好處，特別是可用於公司營運上的項目，十分推薦。",
    "Learn more ↗" : "深入瞭解 ↗",
    ...
    "Set log level to debug" : "設定記錄層級至除錯",
    ...
    "Your web server is not properly set up to resolve \"{url}\". This is most likely related to a web server configuration that was not updated to deliver this folder directly. Please compare your configuration against the shipped rewrite rules in \".htaccess\" for Apache or the provided one in the documentation for Nginx at it's {linkstart}documentation page ↗{linkend}. On Nginx those are typically the lines starting with \"location ~\" that need an update." : "您的網頁伺服器未正確設定，無法解析「{url}」。這可能與伺服器的組態設定，並未更新為直接傳送此資料夾有關。請對應查看您的組態設定，例如 Apache 中的「.htaccess」檔案，請比較一下附上的重寫規則；或者 Nginx，則請查看 {linkstart} 文件頁面 ↗{linkend} 中所提供的文件。在 Nginx 環境中，通常是在「location ~」開頭的那行需要調整更新。",
    ...
    "This is the unsupported community build of Nextcloud. Given the size of this instance, performance, reliability and scalability cannot be guaranteed. Push notifications are limited to avoid overloading our free service. Learn more about the benefits of Nextcloud Enterprise at {linkstart}https://nextcloud.com/enterprise{linkend}." : "這是沒有得到支援的 Nextcloud 社群建置版。鑑於此站台的大小，無法保證效能、可靠程度與延展性。推播通知有所限制，以避免我們的免費服務負載過重。請前往 {linkstart}nextcloud.com/enterprise{linkend} 了解更多 Nextcloud 企業版的好處。",
    "Memcached is configured as distributed cache, but the wrong PHP module \"memcache\" is installed. \\OC\\Memcache\\Memcached only supports \"memcached\" and not \"memcache\". See the {linkstart}memcached wiki about both modules ↗{linkend}." : "Memcached 組態設定為分散式快取，但是安裝了錯誤的 PHP 模組「memcache」。\\OC\\Memcache\\Memcached 僅支援「memcached」而非「memcache」。請參閱 {linkstart}memcached wiki 了解兩種模組的資訊 ↗{linkend}。",
    "Some files have not passed the integrity check. Further information on how to resolve this issue can be found in the {linkstart1}documentation ↗{linkend}. ({linkstart2}List of invalid files…{linkend} / {linkstart3}Rescan…{linkend})" : "部份檔案未通過完整性檢查。關於如何解決此問題的更多資訊，可參閱 {linkstart1} 文件 ↗{linkend}。（{linkstart2} 無效檔案列表…{linkend} / {linkstart3}重新掃描…{linkend}）",
    "The PHP OPcache module is not properly configured. See the {linkstart}documentation ↗{linkend} for more information." : "PHP OPcache 模組的組態未正確設定。請參閱 {linkstart}說明文件 ↗{linkend} 了解更多資訊。",
    ...
    "The PHP modules \"gmp\" and/or \"bcmath\" are not enabled. If you use WebAuthn passwordless authentication, these modules are required." : "未啟用 PHP 模組「gmp」與「bcmath」。若您要使用 WebAuthn 無密碼身份認證，這些模組為必要。",
    "Module php-imagick in this instance has no SVG support. For better compatibility it is recommended to install it." : "站台安裝的 php-imagick 不支援 SVG。為了更好的相容性，建議補安裝。",
    "Some columns in the database are missing a conversion to big int. Due to the fact that changing column types on big tables could take some time they were not changed automatically. By running \"occ db:convert-filecache-bigint\" those pending changes could be applied manually. This operation needs to be made while the instance is offline. For further details read {linkstart}the documentation page about this ↗{linkend}." : "資料庫的有些欄位缺少 big int 格式轉換。由於大行表格中的欄位類型轉換需要一些時間，所以沒有自動轉換。您可以執行「occ db:convert-filecache-bigint」手動套用轉換，轉換作業時 Nextcloud 站台必須處於離線狀態。詳情請參閱 {linkstart} 此問題的相關文件頁面 ↗{linkend}。",
    "SQLite is currently being used as the backend database. For larger installations we recommend that you switch to a different database backend." : "目前後端資料庫使用 SQLite。在大型服務當中，我們建議您使用其他的資料庫後端。",
    "This is particularly recommended when using the desktop client for file synchronisation." : "若您使用桌面客戶端來同步檔案，非常建議您這麼做。",
    "To migrate to another database use the command line tool: \"occ db:convert-type\", or see the {linkstart}documentation ↗{linkend}." : "若要轉移至另一個資料庫，請使用命令列工具：「occ db:convert-type」，或請參閱 {linkstart} 文件 ↗{linkend}。",
    ...
    "MySQL is used as database but does not support 4-byte characters. To be able to handle 4-byte characters (like emojis) without issues in filenames or comments for example it is recommended to enable the 4-byte support in MySQL. For further details read {linkstart}the documentation page about this ↗{linkend}." : "您的 MySQL 資料庫並不支援 4-byte 的字元。為了能處理檔案名稱中，或是留言中的 4-byte 的字元（如表情符號等）。建議您啟用 MySQL 中支援 4-byte 的字元的功能。詳情請見{linkstart}文件中關於此項目的說明 ↗{linkend}。",
    "This instance uses an S3 based object store as primary storage. The uploaded files are stored temporarily on the server and thus it is recommended to have 50 GB of free space available in the temp directory of PHP. Check the logs for full details about the path and the available space. To improve this please change the temporary directory in the php.ini or make more space available in that path." : "此站台使用 Amazon S3 物件儲存為主要儲存空間。上傳檔案會暫存在伺服器，因此建議 PHP 的暫存資料夾最少要有 50 GB 可用容量。請在紀錄檔中查閱關於資料夾路徑及可用容量的詳細資訊。若要增進效能，請在 php.ini 中變更暫存資料夾位置，或將該資料夾容量增加。",
    "The temporary directory of this instance points to an either non-existing or non-writable directory." : "此站台的暫存目錄，指向一個不存在或無法寫入的目錄。",
    ...
    "This instance is running in debug mode. Only enable this for local development and not in production environments." : "此站台以除錯模式執行。請僅在本機開發、且非生產環境時才啟用。",
    ...
    "new" : "新",
    "_download %n file_::_download %n files_" : ["下載 %n 個檔案","下載 %n 個檔案"],
    ...
    "_The update was successful. Redirecting you to {productName} in %n second._::_The update was successful. Redirecting you to {productName} in %n seconds._" : ["更新成功。將在 %n 秒後重新導向至 {productName}。","更新成功。將在 %n 秒後重新導向至 {productName}。"],
    "Applications menu" : "應用程式選單",
    "More apps" : "更多應用程式",
    "Currently open" : "目前開啟",
    "_{count} notification_::_{count} notifications_" : ["{count} 個通知","{count} 個通知"],
    ...
    "Error opening the user status modal, try hard refreshing the page" : "開啟使用者狀態的模態時發生問題，嘗試重新整理頁面",
    ...
    "_Please enter {minSearchLength} character or more to search_::_Please enter {minSearchLength} characters or more to search_" : ["請輸入 {minSearchLength} 個字元以上以搜尋"],
    ...
    "read-only" : "唯讀",
    "_{count} file conflict_::_{count} file conflicts_" : ["{count} 個檔案衝突","{count} 個檔案衝突"],
    ...
    "Enhanced security is enabled for your account. Choose a second factor for authentication:" : "您的帳號已啟用進階安全機制，請選擇一個雙因素認證方法：",
    "Could not load at least one of your enabled two-factor auth methods. Please contact your admin." : "無法載入任何一個雙因素認證方法。請聯絡管理員。",
    "Two-factor authentication is enforced but has not been configured on your account. Contact your admin for assistance." : "雙因素認證已強制要求啟用，但您的帳號目前還沒設定。請聯絡管理員。",
    "Two-factor authentication is enforced but has not been configured on your account. Please continue to setup two-factor authentication." : "雙因素認證已強制要求啟用，但您的帳號目前還沒設定。請繼續設置您的雙因素認證。",
    "Set up two-factor authentication" : "設定雙因素認證",
    "Two-factor authentication is enforced but has not been configured on your account. Use one of your backup codes to log in or contact your admin for assistance." : "雙因素認證已強制要求啟用，但您的帳號目前還沒設定。請使用您的備用碼登入，或聯絡管理員協助。",
