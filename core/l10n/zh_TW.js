OC.L10N.register(
    ...
    "Last background job execution ran {relativeTime}. Something seems wrong. {linkstart}Check the background job settings ↗{linkend}." : "上次背景工作是於 {relativeTime} 前執行，似乎發生了一點錯誤。{linkstart} 請檢查背景工作設定 ↗{linkend}。",
    ...
    "Module php-imagick in this instance has no SVG support. For better compatibility it is recommended to install it." : "站台安裝的 php-imagick 不支援 SVG。建議安裝以獲更好的相容性。",
    "Some columns in the database are missing a conversion to big int. Due to the fact that changing column types on big tables could take some time they were not changed automatically. By running \"occ db:convert-filecache-bigint\" those pending changes could be applied manually. This operation needs to be made while the instance is offline. For further details read {linkstart}the documentation page about this ↗{linkend}." : "資料庫的有些欄位缺少 big int 格式轉換。由於 big tables 中的欄位類型轉換需要一些時間，所以沒有自動轉換。您可以執行「occ db:convert-filecache-bigint」手動套用轉換，轉換作業時 Nextcloud 站台必須處於離線狀態。詳情請參閱 {linkstart} 此問題的相關文件頁面 ↗{linkend}。",
